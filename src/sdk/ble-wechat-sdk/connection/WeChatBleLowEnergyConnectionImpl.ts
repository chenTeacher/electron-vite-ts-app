...
import {WechatBle} from '../util/WechatBle'

export class WeChatBleLowEnergyConnectionImpl implements BleConnectionInterface {
					...
					if (dev.isRecover != undefined && dev.isRecover) {
						...
						this.ble.createBLEConnection({ deviceId: deviceId }, (res: any) => {
							...
							if (res.status == '2500') {
								...
								//协调mtu
								this.ble.coordinateBLEMTU({deviceId: deviceId},(mtuRes:any)=>{
									if (mtuRes.status == '2500') {
										let dev = this.deviceList[devIndex];
										dev.mtu=mtuRes.value
										this.updateDevice(dev);
									}
								})
	...
	private initListener(): void {
		...
		this.ble.onBluetoothDeviceFound({}, (res) => {
			...
			if (res.status == '2500') {
				let devices: BleDevice[] = res.devices;
		...
		this.ble.onBLEConnectionStateChange({}, (res) => {
			...
			if (connected == true) {
				...
				if (devIndex >= 0) {
					let dev = this.deviceList[devIndex];
					//是否开启了自动重连。如果是手动断开连接为了防止自动重连再次连上。所以通过isAutoReConn进行控制.
					//不允许恢复的设备进行断线重连
	...
	private getDeviceDetail(deviceId: string): void {
		...
		this.ble.getBLEDeviceServices({ deviceId: deviceId }, (res) => {
			if (res.status == '2500') {
				...
				let services: BleService[] = res.services
					...
					if (index < services.length) {
						...
						thit.ble.getBLEDeviceCharacteristics(
							...
							(res2: any) => {
								...
								let characteristics: Characteristic[] = res2.characteristics
	...
	async readMessage(message: BleMessageModel): Promise<string | void> {
		...
		return new Promise((resolve, reject) => {
			this.ble.readBLECharacteristicValue(message, (res) => {
				log.debug('[主动读响应] [readBLECharacteristicValue]' + JSON.stringify(res));
				if (res.status == '2500') {
					this.handleBluetoothData(res.value, message.deviceId);
					resolve(res.value);
				} else {
					reject(res);
				}
    ...
    async publishMessage(message: BleMessageModel): Promise<string | void> {
        ...
        if (!value) {
            return Promise.reject(new Error("数据荷载不能为空"));
        }
        ...
        return new Promise((resolve, reject) => {
            this.ble.writeBLECharacteristicValue(message, (res) => {
                log.debug('[尝试写响应] [writeBLECharacteristicValue]' + JSON.stringify(res));
				if (res) {
					resolve(res.message);
				} else {
	 ...
	 * 发送消息并等待返回
	 */
	async publishMessageWait(message: BleMessageModel, timeout: number,ackCheck: (resHex:string) => boolean,retry: number = 0,allowTransactionHistory:boolean = false): Promise<string | undefined> {
		// 记录开始时间
